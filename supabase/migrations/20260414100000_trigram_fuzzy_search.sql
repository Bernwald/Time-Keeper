...
-- Permission filter (p_user_id) continues the pattern from
-- 20260413100000_permission_model.sql: NULL user → no filter; for a given
-- user, only chunks whose source has no folder OR the user has access to
-- the folder are visible.
-- ---------------------------------------------------------------------------
...
AS $$
  ...
  trgm AS (
    -- Typo-tolerant retrieval. `<%` uses word_similarity with the
    -- threshold set via SET above (0.4 is lenient enough to catch 1-char
    -- typos like Elfach/Elfack without flooding with false positives).
    SELECT c.id,
  ...
  combined AS (
    -- Reciprocal Rank Fusion across all three retrieval arms. UNION ALL is
    -- cleaner than chained FULL OUTER JOINs when merging N sources.
...
-- ---------------------------------------------------------------------------
-- hybrid_search_boosted — same, plus a per-source boost factor for entity
-- resolution. Boost is applied after RRF so it scales the fused score.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- search_chunks — FTS-only fallback (no embedding available). Also gets a
-- trigram OR-branch so typos still surface results without embeddings.
-- ---------------------------------------------------------------------------
