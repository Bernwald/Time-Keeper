...
Schritte, damit gebrandete Login-/Invite-Mails aus hAIway verschickt werden. Absender-Domain: `timekeeper.bernwald.net`.
...
Speichern. Danach **Send Test Email** (unten auf derselben Seite) an deine eigene Adresse — die Mail sollte mit `From: hAIway <noreply@timekeeper.bernwald.net>` ankommen.
...
2. **Subject** setzen (Vorgabe im HTML-Kommentar oben):
   - Magic Link → `Ihr Anmelde-Link für hAIway`
   - Invite user → `Sie wurden zu hAIway eingeladen`
   - Confirm signup → `Bestätigen Sie Ihr hAIway-Konto`
   - Reset password → `hAIway: Passwort zurücksetzen`
...
Die Migration `20260414130000_org_is_platform.sql` setzt `is_platform = TRUE` für die hAIway-Org. Ohne die Migration schlagen die Admin-Queries fehl (`column organizations.is_platform does not exist`).
